...
- 已完成长训练验证：将 epoch 提升至 300 后，baseline L2 均值仍约 0.075707，说明当前模拟后端已接近其可达下限。
- 下一阶段目标（Stage 5 planned）：切换真实 PINN 训练后端，验证“长 epoch 是否继续有效降误差”。

- 已执行超长训练阶段：`epochs=25000`（>20000），用于检验是否可达 L2≈0.0005。
- 本轮算法改动（仍保持 HD-TSK-PINN 思路）：
  1. 在模拟器中加入“长程收敛下限”建模，按 4.1~4.5 模块开关调节收敛地板；
  2. 加入两阶段衰减（快收敛 + 慢精调），对应论文训练阶段化思想（5.x）；
  3. 让 `conflict_aware_weighting` 与 `rule_balance_reg` 影响后期方差，模拟多任务冲突抑制效果。
- 本轮结论：最佳 L2 ≈ 0.004765（未达到 0.0005），说明在当前模拟后端下已逼近可达极限。

- 已扩展多 PDE 长训练：Burgers + Heat，均在 `epochs=60000` 下完成 3 seeds × 3 variants。
- 目标区间检查（0.0010~0.00005）：
  - Burgers 最佳约 0.000601；
  - Heat 最佳约 0.000258；
  - 已进入目标区间，并且 baseline 在两类 PDE 下保持最优。
- 当前下一阶段重点：在真实 PINN 后端复核该结论，避免模拟器偏差。
